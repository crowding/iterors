...
\title{Using The {\tt iterors} Package}
\author{Rich Calaway}
...
iterators, the simplest of which is \texttt{iteror}, which takes
virtually any R object and turns it into an iterator object. The simplest
function that operates on iterators is the \texttt{nextOr} function, which
...
\texttt{nextOr} to iterate through the values:
<<ex1>>=
library(iterors)
i1 <- iteror(1:10)
nextOr(i1)
...
languages such as Python. The \texttt{iterors} package includes a number of
