...
has \texttt{iter} and \texttt{nextOr} methods.  The purpose of the
...
that don't define a \texttt{nextOr} method.  We call those objects
...
nextOr(it)
nextOr(it)
tryCatch(nextOr(it), error=function(e) e)
@
...
made it sound like you have to write your own \texttt{iteror} and
\texttt{nextOr} methods, you can inherit them.  In fact, that's what
all of the following examples do.  I do that by inheriting
from the \texttt{iteror} class.  The \texttt{abstractiter} class uses
the standard \texttt{iteror} method which returns itself, and defines a
\texttt{nextOr} method that calls the \texttt{nextOr} element of the object.
Let's take a look at the implementation of these two methods:

<<nextOr.abstractiter>>=
iterators:::iteror.iteror
iterators:::nextOr.iteror
@
...
function that gets called by the \texttt{nextOr} method.  This function not
...
indirectly, by calling \texttt{nextOr} on the iterator created via the
