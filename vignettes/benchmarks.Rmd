...
This document illustrates the differences in performance between `iterors` and previous packages `iterators`/`itertools`/`itertools2`.  In compiling  `iterors` package several functions have been optimized. Other gains come from the `nextOr` method  avoiding the need for tryCatch.
