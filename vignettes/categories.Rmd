...
`iteror` -- Generic function to convert an object into an iteror

## Consuming / summarizing an iteror

###TODO: these should be s3 generic if they are not already.

`as.character`, `as.numeric`, `as.logical`, `as.list` -- Collect all an iterator's values into the named type of vector.
`as.vector` -- Specify the type of vector as an argument.
`nextOr` -- Take the next element from an iterator.
`take` -- Take some number of elements and return a vector.
...
`record` -- Write an iterator's elements to a file.
...
`reduce` -- Combine all of iterator's values using a binary function, returning the final value.
`sum.iteror` -- `reduce` using `+`.
`prod.iteror` -- `reduce` using `*`.

## Iterate over given data

By convention, in this package functions that _construct_ an iterator have names beginning with "`i`".

Many of these functions also have options for chunking and repeating the contents.
...
Many of these functions also accept options for chunking and repeating their contents.

`icombinations` -- Enumerate all combinations of $m$ elements of the given vector, with or without replacement.
`icount` -- Produce an integer series starting with 1.
`icountn` -- Count along multiple indices in row-major or col-major order.
`idiv` -- Return $n$ integers that add up to a given total.
...
`iseq` -- Generate arithmetic sequences with customizable step size and origin.
`iseq_along` -- Counting sequence parallel to a given vector.

## Random number generation

Random number iterators can be made reproducible by specifying `independent=TRUE` and specifying a seed.
...
`record` -- Write values from an iterator to a file. `ireplay`
...
`[i_window]` -- Return adjacent elements from the underlying iteror in a sliding window, advancing the window by one step.

### Splitting and combining

`i_chain` -- Take all values
`i_collapse` -- Take all values from the list of
