...
Suppose you iterate over this list using NULL as your end-of-iteration sigil, like this:
...
`i_rep`, and give it another argument called `times`:
...
The differences between `iforever` and `i_rep` are in the inner
...
of variables that were passed as arguments to `i_rep`, and it can
...
using the `i_rep` function, and they will all work as expected without
conflicts.

Note that this iterator only uses the arguments to `i_rep` to
store its state.  If any other state variables are needed, they can be
defined anywhere inside the `i_rep` function.
...
Note that this looks an awful lot like the `i_rep` function that
we implemented previously.  In fact, using `i_limit`, we can
implement `i_rep` using `iforever` much more simply, and
without duplication of code:

```{R i_rep2}
i_rep2 <- function(x, times)
  i_limit(iforever(x), times)
```

To demonstrate `i_rep2`:
