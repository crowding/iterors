...
### Detecting end of iteration with a sigil value
...
is often called a _sigil value_.

#### Unsafe sigil values

It's popular in R to use `NULL` as a sigil, but it's worth considering
what would happen if there were an iterator that legitimately returns
...
If you were to try to iterate over this list, like:
...
value in the list that we are iterating over, for `NULL` the sigil
value. This is a general problem with the sigil value scheme in
...
sigil value to use locally, just with this iteror, and then throw
away, not keeping it in your code. One easy way to do this by using
`new.env()`. Consuming an iterator using a local sigil value looks
like this:
...
it <- iteror(bad_sigils)
repeat {
  val <- nextOr(it, end_sigil)
  if (identical(val, end_sigil)) break
  print(val)
}
```
...
pointer comparison.) By using a locally generated, unique sigil value
