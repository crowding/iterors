...
test_that("istarmap over the rows of a data.frame", {
  ...
  expect_equal(nextElemOr(it, NA), sum(iris[1, -5]))
  expect_equal(nextElemOr(it, NA), sum(iris[2, -5]))
  expect_equal(nextElemOr(it, NA), sum(iris[3, -5]))
  expect_equal(nextElemOr(it, NA), sum(iris[4, -5]))
  expect_equal(nextElemOr(it, NA), sum(iris[5, -5]))
  expect_equal(nextElemOr(it, NA), sum(iris[6, -5]))
  # and so on...
})
