test_that("block matrix multiplication", {
 ...
 library(foreach)
  ...
  ## ours <- iteror(test, by=1, chunks=3) |> iapply(iteror, by=2, chunks=4)
  ## ou <- ours()
  ## th <- theirs()
  ...
  actual <- foreach(ia=(iteror(x, by=1, chunks=xchunks) |>
                          iapply(iteror, by=2, chunks=chunks)),
                    .combine=rbind) %:%
    foreach(a=ia, ib = (iteror(y, by=1, chunks=chunks) |>
                          iapply(iteror, by=2, chunks=ychunks)),
            .combine=`+`) %:%
  ...
  actual <- foreach(ib=(iteror(y, by=2, chunks=ychunks) |>
                          iapply(iteror, by=1, chunks=chunks)),
                    .combine=cbind) %:%
    foreach(b=ib, ia=(iteror(x, by=2, chunks=chunks) |>
                        iapply(iteror, by=1, chunks=xchunks)),
            .combine=`+`) %:%
