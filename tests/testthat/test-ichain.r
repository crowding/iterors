context("ichain iterator")

test_that("ichain properly iterates through multiple numeric vectors", {
  it <- ichain(1:3, 4:5, 6)
  expect_equal(nextElemOr(it, NA), 1)
  expect_equal(nextElemOr(it, NA), 2)
  expect_equal(nextElemOr(it, NA), 3)
  expect_equal(nextElemOr(it, NA), 4)
  expect_equal(nextElemOr(it, NA), 5)
  expect_equal(nextElemOr(it, NA), 6)
  expect_equal(nextElemOr(it, NA), NA)
})

test_that("ichain properly iterates through a numeric vector and a character vector", {
  it <- ichain(1:3, levels(iris$Species))
  expect_equal(nextElemOr(it, NA), 1)
  expect_equal(nextElemOr(it, NA), 2)
  expect_equal(nextElemOr(it, NA), 3)
  expect_equal(nextElemOr(it, NA), "setosa")
  expect_equal(nextElemOr(it, NA), "versicolor")
  expect_equal(nextElemOr(it, NA), "virginica")
  expect_equal(nextElemOr(it, NA), NA)
})

test_that("ichain properly iterates through a numeric vector and a data.frame's columns", {
  it <- ichain(1:3, iris)
  expect_equal(nextElemOr(it, NA), 1)
  expect_equal(nextElemOr(it, NA), 2)
  expect_equal(nextElemOr(it, NA), 3)
  expect_equal(nextElemOr(it, NA), iris[, 1])
  expect_equal(nextElemOr(it, NA), iris[, 2])
  expect_equal(nextElemOr(it, NA), iris[, 3])
  expect_equal(nextElemOr(it, NA), iris[, 4])
  expect_equal(nextElemOr(it, NA), iris[, 5])
  expect_equal(nextElemOr(it, NA), NA)
})
