...
Because this is a more computationally intensive vignette, it is built manually and included in the released package statically.  The `Rmd` source is in the Git repo under the `scratch` directory. When this document was last built it was under these conditions: 
...
* iterators version `r packageVersion("iterors")`; itertools version `r packageVersion("itertools")`; itertools2 version `r packageVersion("itertools2")`
...
first check whether those functions are roughly on par.
